...
def process_csv_files(folder_path):
    ...
    if not dataframes:
        print("no valid Ccsv files to process.")
        return
...
def calculate_rmse_counts(detections, ground_truth_counts):
    ...
    for image_name, predicted_apples in detections.items():
        ...
        gt_count = ground_truth_counts.get(image_name.split('/')[1].replace('RGBhr','RGB'), 0)
        error = (predicted_count - gt_count)**2
        errors.append(error)

        per_image_rmse[image_name.split('/')[1].replace('RGBhr','RGB')] = np.sqrt(error)

    total_rmse = np.sqrt(np.mean(errors)) if errors else 0.0
    return per_image_rmse, total_rmse

#distance between two circle
def distance(c1, c2):
    x1, y1, _ = c1
    x2, y2, _ = c2
    return math.sqrt((x1 - x2)**2 + (y1 - y2)**2)

#this is needed for iou calc, neares detect to nearest actual
def match_apples_by_min_distance(predicted_circles, ground_truth_circles):
    ...
    #greedy match pick the first available pair
